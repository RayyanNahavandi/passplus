...
export default function Home() {
  ...
  useEffect(() => {
    fetch("/api/presence")
      .then((r) => r.json())
      .then((data: { count?: number }) => setStudyingNow(data.count ?? 0))
      .catch(() => {})
  }, [])
    ...
    <main className="flex flex-col min-h-screen bg-background">
        ...
        <div className="flex flex-col items-center gap-7 max-w-3xl w-full">
          ...
          {studyingNow >= 3 && (
            <motion.p
              ...
              className="flex items-center gap-2 text-xs text-muted-foreground bg-card border border-border rounded-full px-3.5 py-1.5"
            >
              <span className="relative flex w-2 h-2" aria-hidden>
                <span className="absolute inline-flex h-full w-full rounded-full bg-accent-green opacity-60 animate-ping motion-reduce:animate-none" />
                <span className="relative inline-flex w-2 h-2 rounded-full bg-accent-green" />
              </span>
              {studyingNow} {studyingNow === 1 ? "person" : "people"} studying right now
            </motion.p>
          )}
      ...
      <footer className="px-6 py-5 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-muted-foreground/50">
        ...
        <span className="flex items-center gap-4">
          <Link href="/help" className="text-accent-green hover:text-accent-hover transition-colors">
            Help
          </Link>
